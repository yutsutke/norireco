...
**`CACHE_VERSION = 'v328'`** · デプロイ回数 = バージョン番号の不変式
...
| **駅 id 体系 Phase 3-k: LINES id 付与カバレッジ 100% (merged_stations 13 駅補完)** (v328) | ✅ 完成 — `tools/add_missing_13_stations.js` で lines-p2.json の座標を流用して 13 駅 (s_09018〜s_09030) を merged_stations に追加 (常磐線 11 + 山陽線 2 + 東北線 1)。add_line_station_ids.js 再実行で **カバレッジ 99.87% → 100.00%** (far=0, missing=0)。SERVICE_LINES (jr_joban_medium 等) への追加は別スコープ (🟢 データ充実)。CHANGELOG §178 |
