...
**`CACHE_VERSION = 'v403'`** · デプロイ回数 = バージョン番号の不変式
...
| **Phase 3.8 一括記録 (A カテゴリ)** (v400〜) | 🟡 進行中 — Notion §1.3 設計確定「営業系統チェックリスト + たたむ/開くアコーディオン」。B カテゴリ (`createTripDetailEditor` 3 mode + multi-container API) が前提として揃った上での A 本体着手。A-1 (v400) skeleton + A-2 (v401) チェックリスト本体 + A-3 (v402) 一括保存 MVP 完了。**A-4 (v403)**: 検索 + 並び替え + 地域フィルタ — `_filter = { query, sort: 'near'|'name', group }` state、検索 input (系統名/会社/id 部分一致、空白 AND)、並び替え `近く順 (lastUserGps > map center 優先)` / `名前順 (50 音)`、地域 group dropdown (13 値)。フィルタバーは触らず `_renderChecklistOnly()` で checklist 領域だけ書換 = IME 安定パターン (mp-trip-filter と同設計)。チェック状態は filter 変更で保持。preview で「山手」検索→2 件 / 「JR東日本 新幹線」AND→2 件 / 「関西」絞り込み→134 件 + 50 音順切替 / リセット動作 / チェック保持 全 OK、console error 0。残 A-5 (アコーディオン展開 + 環状線半周分割) → A-6 (オンボーディングバナー) → A-7 (unknown 集計検証) |
