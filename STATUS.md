...
**`CACHE_VERSION = 'v399'`** · デプロイ回数 = バージョン番号の不変式
...
| **Phase 3.8 trip 詳細エディタ抽出 (B カテゴリ)** (v392〜v399) | ✅ 完成 — Notion §1.3「一括記録」設計判断より、確認モーダル中身の 02/07/13b 3 箇所重複 (v383 落とし穴) を解消する単一 factory `createTripDetailEditor` に段階リファクタで集約完了。B-1 (v392, per-seg-chip mode 本実装 + 07 切替) → B-2 (v393, 13b を per-seg-rows / trip-level mode 移行) → B-3a (v394, 13b 遅延/メモ集約) → v395 hotfix (delay/notes リロード消失バグ修正) → B-3b (v396, 13b 時刻集約 2 精度) → B-3c (v397, 07 へ 5 精度 + maniaToggle 移植) → B-4-a (v398, visible dead code ~350 行撤去) → **B-4-b (v399)**: グローバル `NORIRECO.trains.selectedXxxBySl / activeChipSlId / selectedTrainId-Name-Category-CarModel` 9 fields 撤廃 + 02 旧 cascade handler (`resetTrainSelector / onTrainCategoryChange / onTrainChange / onTrainCustomInput / onCarModelChange / onCarModelCustomInput` ~190 行) + 07 旧 SL chip ロジック (`applyRecTrainCategory / clearAllTrainSelections / populateSlVehiclePicker / selectSlChip / populateSlVehicleSelect / onSlVehicleChange / isInDropdown / onSlVehicleCustomInput` ~330 行) 撤去 + factory に `containers: { time, train, delay, notes, photos }` multi-container API 追加 + 各 modal **3 instance → 1 instance** に統合 (07 `_recEditor` / 13b `_tripEditEditor`)、HTML の `#rec-meta-container` / `#trip-edit-meta-container` を delay/notes 独立コンテナに分割。**正味 ~540 行削除**、可読性大幅向上。詳細 → CHANGELOG.md §242〜§249。次は **A (一括記録パネル本体着手)** — Notion §1.3 設計確定済、factory が揃ったので一気に作れる |
