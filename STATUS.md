...
**`CACHE_VERSION = 'v423'`** · デプロイ回数 = バージョン番号の不変式
...
| 垢 BAN（シェア停止ペナルティ） | 🟡 本体実装済 (v423) — `norireco_profiles` (share_status が真実の源・本人も自己解除不可な RLS) + `shares.revoked` 派生キャッシュ + INSERT/UPDATE policy 強化 + 関数 EXECUTE を public REVOKE + banned 時シェアモーダル不開 (画像シェア/DL も一括ブロック) + マイページ状態バナー/チップ + 配信側 `&revoked=is.false`。trip/grant・Worker 不触 (布石 #6)。発動=Dashboard 手動 SQL (`ban_user_share`/`unban_user_share`)。**SQL Run + push 待ち** (SQL 先→JS 後)。管理 GUI/自動発動/full_banned 個人記録停止は別タスク。詳細 → CHANGELOG §273 |
| マップ情報レイヤー構想 | ❌ 未着手（将来） |
| 廃線対応 | ❌ 未着手（🎮 将来） |
