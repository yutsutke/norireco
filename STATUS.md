...
**`CACHE_VERSION = 'v332'`** · デプロイ回数 = バージョン番号の不変式
...
| **駅 id 体系 Phase 3-h: norireco_memos.station 列廃止 + backfill ヘルパー** (v325) | 🟡 JS 側完了 — `getMemoStationName` 導入、station 列への並行書き込み撤去、`countMemosForStation` 追加、`window.norirecoBackfillMemoStationIds()` で既存 3 件 backfill。**Supabase SQL `v325_memo_station_drop.sql` 実行待ち** (前提: backfill 完遂、v331 で残漏れ修正済)。CHANGELOG §175 |
| **駅 id 体系 Phase 3-i: norireco_trips.from_station/to_station 列廃止 + backfill ヘルパー** (v326) | 🟡 JS 側完了 — `getTripStationName` 導入、name への並行書き込み撤去、findStCoord / alert メッセージを id → name 逆引きに、`window.norirecoBackfillTripStationIds()` で残 NULL 救済。**Supabase SQL `v326_trip_station_drop.sql` 実行待ち** (前提: 全 trip が *_station_id NOT NULL、v331 で残漏れ修正済)。CHANGELOG §176 |
