...
**`CACHE_VERSION = 'v405'`** · デプロイ回数 = バージョン番号の不変式
...
| **Phase 3.8 一括記録 (A カテゴリ)** (v400〜v405) | ✅ 完成 — Notion §1.3 設計確定「営業系統チェックリスト + たたむ/開くアコーディオン」の本体。A-1 skeleton + A-2 チェックリスト + A-3 一括保存 MVP + A-4 検索/フィルタ + A-5 アコーディオン展開 (createTripDetailEditor 行内 mount、同時 1 行制御、編集 ✏️ マーク) + **A-6+A-7 (v405)**: 空マップ時オンボーディングバナー (旅程 0 件で gold バナー + 矢印アニメーション、タップで bulk sheet 起動、`updateOnboardingBanner` を bulk 保存後 + 通常記録 saveMultiSegmentTrip 後 + DOMContentLoaded で呼ぶ) + unknown 集計検証 (現状 (b) で確定: 期間フィルタ specific で unknown trip は地図塗りからも除外。「期間フィルタ = 日時判明 trip 内で絞る」という意味論で整合、Notion §1.3 採用 (a) → (b) に更新)。**環状線対応** (山手線 17/30 駅塗り問題) は A スコープ外として別タスクへ持ち越し (N02 line データ補完 = Phase 3 駅 ID 体系の延長作業)。詳細 → CHANGELOG.md §250〜§255 |
