...
**`CACHE_VERSION = 'v307'`** · デプロイ回数 = バージョン番号の不変式
...
| **駅 id 体系 Phase 1 + 用語整理「完駅率」 + slRiddenSt ばらまき撤廃 + 駅クリック確実化** (v290〜v306) | ✅ 完成 — `merged_stations.json` 全 9,017 駅に `s_NNNNN` id 付与、SERVICE_LINES に伝播、集計・描画判定すべて id ベース化 (v293〜v296)。「完乗率」→「完駅率」用語整理 (v297)。slRiddenSt の「candidateN02Ids 経由ばらまき」を撤廃して系統ごとに正確判定 (v298〜v300)。Canvas tolerance 不発環境向けに map.click delegate (40px) で駅クリック確実化 (v304〜v306)。CHANGELOG §138-§154 |
