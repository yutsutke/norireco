...
**`CACHE_VERSION = 'v401'`** · デプロイ回数 = バージョン番号の不変式
...
| **Phase 3.8 一括記録 (A カテゴリ)** (v400〜) | 🟡 進行中 — Notion §1.3 設計確定「営業系統チェックリスト + たたむ/開くアコーディオン」。B カテゴリ (`createTripDetailEditor` 3 mode + multi-container API) が前提として揃った上での A 本体着手。A-1 (v400) skeleton 完了。**A-2 (v401)**: 営業系統チェックリスト本体 + たたむモード — `_bulkDrafts: Map<lineId, draft>` で内部 state 管理、638 系統 (まだフィルタなし、A-4 で追加) を全件描画、チェック = 全線 1 segment の draft 1 件を push (`source=manual, verified=false, date_precision='unknown'`、`from`=stations[0].name `to`=stations[-1].name + 駅 id も付与)、アンチェック = Map.delete。サマリバーに「N 件選択中」とリアクティブ更新、保存ボタンは A-3 まで disabled の骨だけ。環状線は 🔄 マーク付き (山手線 = 東京→有楽町 で visit-only 相当、A-5 で半周 2 segment 分割を検討)。preview eval で 2 件チェック / 1 件アンチェック / close cleanup を全検証 OK、console error 0。次は A-3 (一括保存 MVP) → A-4 (検索/フィルタ) → A-5 (アコーディオン展開 = factory mount) → A-6 (オンボーディングバナー) → A-7 (unknown 集計検証) |
