...
**`CACHE_VERSION = 'v404'`** · デプロイ回数 = バージョン番号の不変式
...
| **Phase 3.8 一括記録 (A カテゴリ)** (v400〜) | 🟡 進行中 — Notion §1.3 設計確定「営業系統チェックリスト + たたむ/開くアコーディオン」。B カテゴリ (`createTripDetailEditor` 3 mode + multi-container API) が前提として揃った上での A 本体着手。A-1 skeleton + A-2 チェックリスト + A-3 一括保存 MVP + A-4 検索/フィルタ完了 (v400〜v403)。**A-5 (v404)**: アコーディオン展開 — 行右端 ▶/▼ ボタンで `createTripDetailEditor` (`trainPicker='per-seg-rows'` + 5 精度 time + delay + notes、photos は将来) を multi-container API で行内 mount。Notion §1.3 確定の「同時 1 行制御」: 別の行を開くと現開行は close 時に `editor.getDraft()` で `_bulkDrafts` 上書き (`_edited: true` フラグ) + destroy。`_buildTripFromDraft` を `_edited` 分岐対応 (date/precision/depart/arrive/segments/delay/notes 上書き + 集約ルールで trip 直下 train_* も埋める)。編集済み行は名前に ✏️ マーク。preview で展開→入力 (delay 90分 + notes)→別行展開で close→上書き→保存→trip 反映 全 OK、console error 0。**環状線対応**: 半周分割を検証したが SERVICE_LINES と N02 line の駅順ズレで根本解決せず、A-5 スコープ外として別タスクへ持ち越し (山手線 17/30 駅塗りの既知制約として bulk-note に明記済)。残 A-6 (オンボーディングバナー) → A-7 (unknown 集計検証) |
