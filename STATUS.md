...
**`CACHE_VERSION = 'v425'`** · デプロイ回数 = バージョン番号の不変式
...
| 垢 BAN（シェア停止ペナルティ） | 🟡 本体 (v423) + full_banned enforcement (v424) + 旧 ALL policy 穴塞ぎ (v425) — v423: `norireco_profiles` (share_status が真実の源・本人も自己解除不可な RLS) + `shares.revoked` 派生キャッシュ + INSERT/UPDATE policy 強化 + 関数 EXECUTE を public REVOKE + banned 時シェアモーダル不開 + マイページ状態バナー/チップ。**v424**: trip/character_grant/memo の INSERT policy に `NOT EXISTS(full_banned)` 追加 (UPDATE/DELETE/SELECT は据え置き) + 各 INSERT 呼び元 (07/21/16/03) の冒頭に full_banned ガード + バナー/チップ文言を share_banned/full_banned で分岐 → 段階の差別化 (share_banned=シェアのみ停止 / full_banned=シェア + 新規記録停止 + 過去記録は閲覧編集可) が完成。**v425 補修**: v424 Run 後の確認で `norireco_trips` に旧 `FOR ALL` policy 1 件が残留と判明 (v135〜v250 頃の歴史的残骸、v421 個別 cmd 名 DROP で名前漏れ)。RLS は PERMISSIVE = OR 評価のため v424 full_banned ガードが空転していた → DO ブロックで ALL を冪等 DROP。trip/grant・Worker 不触 (布石 #6)。発動=Dashboard 手動 SQL (`ban_user_share`/`unban_user_share`/`set_account_status`)。**v425 SQL Run 待ち**。管理 GUI/自動発動 (スパム量/通報) は別タスク。詳細 → CHANGELOG §273-§275 |
| マップ情報レイヤー構想 | ❌ 未着手（将来） |
| 廃線対応 | ❌ 未着手（🎮 将来） |
