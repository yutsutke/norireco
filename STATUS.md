...
**`CACHE_VERSION = 'v400'`** · デプロイ回数 = バージョン番号の不変式
...
| **Phase 3.8 一括記録 (A カテゴリ)** (v400〜) | 🟡 進行中 — Notion §1.3 設計確定「営業系統チェックリスト + たたむ/開くアコーディオン」。B カテゴリ (`createTripDetailEditor` 3 mode + multi-container API) が前提として揃った上での A 本体着手。**A-1 (v400)**: skeleton — `js/21-bulk-record.js` 新設 (open/close API + window 公開)、`#bulk-record-sheet` モーダル枠を noritetsu-map.html に追加 (`.memo-modal` ボトムシート流用)、マイページ旅程サブタブ上部に `.mp-bulk-entry` 常設ボタン (13b-trips.js renderMpTripsSection 先頭に挿入 + import で 21-bulk-record.js を間接ロード)。preview server で open/close + body innerHTML 書換 + ボタン onclick wiring を eval 検証 OK。次は A-2 (営業系統チェックリスト本体 + たたむモード = ゼロ摩擦 draft 1 件) → A-3 (一括保存 MVP) → A-4 (検索/フィルタ) → A-5 (アコーディオン展開 = factory mount) → A-6 (オンボーディングバナー) → A-7 (unknown 集計検証) |
