...
**`CACHE_VERSION = 'v437'`** · デプロイ回数 = バージョン番号の不変式 (v437 シェア経由の登録転換 attribution ※ v437_share_referral.sql 要 Run)
