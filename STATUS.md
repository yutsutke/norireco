...
**`CACHE_VERSION = 'v402'`** · デプロイ回数 = バージョン番号の不変式
...
| **Phase 3.8 一括記録 (A カテゴリ)** (v400〜) | 🟡 進行中 — Notion §1.3 設計確定「営業系統チェックリスト + たたむ/開くアコーディオン」。B カテゴリ (`createTripDetailEditor` 3 mode + multi-container API) が前提として揃った上での A 本体着手。A-1 (v400) skeleton + A-2 (v401) チェックリスト本体 + たたむモード完了。**A-3 (v402)**: 一括保存 MVP 完成 — `saveBulkDrafts` で draft 配列を順次 trip 構築 → Supabase POST (anon Bearer、saveMultiSegmentTrip と同経路) → localStorage push (失敗時も継続 = 部分コミット許容) → 全件 push 後に RIDDEN_SEGS bulk push + `NORIRECO.rideRecord.rebuild()` 1 回呼び出し + `redrawAllLinesAfterTripChange` + `updateOverlays` + `_mypageCache.push` + `renderMpTripsResultOnly` + 駅シート refresh + トースト + sheet 自動 close。trip 構造: `id=trip_${baseTime}_${idx}` / `name=${lineName} 全線` / `date_precision='unknown'` / `transfers=0` / `total_stations=stations.length`。preview で 3 件保存 (中央線快速 24 駅 / 上野東京ライン 44 駅 / 湘南新宿ライン 39 駅) → riddenSt 全展開確認 (中央線 31 駅 / 東海道線 35 駅 / 山手線 12 駅) + 完乗率更新確認 + console error 0。**MVP 完成**。次は A-4 (検索 + フィルタ「近く/会社/都道府県」) → A-5 (アコーディオン展開 + 環状線半周分割) → A-6 (オンボーディングバナー) → A-7 (unknown 集計検証) |
