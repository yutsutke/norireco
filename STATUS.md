...
**`CACHE_VERSION = 'v329'`** · デプロイ回数 = バージョン番号の不変式
...
| **データ充実: 13 駅を 3 SERVICE_LINES に収録 + isolation_rank 再計算** (v329) | ✅ 完成 — `tools/add_13_stations_to_service_lines.js` で jr_joban_medium に 11 駅追加 (名称も「品川〜原ノ町」→「品川〜岩沼」)、jr_sanyo_main に 英賀保/はりま勝原 を 姫路-網干 間に挿入、jr_tohoku_main_rifu に 陸前山王 を 岩切-新利府 間に挿入。merged_stations の lines/colors も同時更新。compute_isolation_rank.js 再実行で新規 13 駅が rank 3-5 / nearest 2-6.7km に確定 (前回は 0 / null)、隣接駅情報なし 0 駅 (9030 駅全てが何らかの SERVICE_LINE に収録)。CHANGELOG §179 |
