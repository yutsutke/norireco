...
## ドキュメント地図（どこに何があるか） — **本ドキュメントが真実の源**（v275 で集約）

| ドキュメント | 役割 | 更新頻度 |
|---|---|---|
| **STATUS.md** | 現在のスナップショット（CACHE_VERSION / 領域別ステータス / 直近フェーズ） | ターンごと・git |
| **CHANGELOG.md** | 変更履歴詳細（現行 Phase） | ターンごと・git |
| **TODO.md** | やること（🔥🟡🟢🔧🎮 分類） | 着手/完了時・git |
| **CLAUDE.md**（このファイル） | 規約・コーディング・運用ルール（指針） | 規約変更時・git |
| **`.claude/hooks/*.js`** | 強制（SessionStart inline / Stop reminder） | hook 変更時・git |
| **Notion 乗レコ全体像** (id `36171b458b638120b572fed38140e054`) | 仕様詳細（§1.x 画面 / §2.x 裏側 / §2.6 データの流れ / §2.7 命名辞書 / §2.5 落とし穴）・大方針（§0.2）・戦略（§3） | セッション末・Notion |
| **CHANGELOG_PHASE*.md** | 過去フェーズアーカイブ（Phase 1-3.7 / 3.8-early / 3.8-modules） | read-only |

- 同じ内容を 2 か所に書かない（v274 で確定 / v275 で本表に集約）
