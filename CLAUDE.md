...
## 原則
- 真実の源はコード。ドキュメントとずれたらコードが正しい → §2.7.2 意思決定ログに記録
...
- 影響を受けた子ページ（§1.x / §2.x / §2.7 系）を現状に整合
- 新語は §2.7.1 用語集 に追記、設計判断は §2.7.2 意思決定ログ に追記、既存と矛盾しないか確認（§2.7 本体は索引・基本軸・未統一のみで軽量化済 — v414）
...
- §2.7 命名辞書は 3 ページ構成 (v414 整理): §2.7 本体（索引）/ §2.7.1 用語集 / §2.7.2 意思決定ログ
- コードとドキュメントがずれたらコードが正しい → Notion §2.7.2 意思決定ログに記録
