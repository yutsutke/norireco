...
## ドキュメント地図（どこに何があるか）
- **STATUS.md** = 現在のスナップショット（CACHE_VERSION / 領域別ステータス / 直近フェーズ）。ターンごとに最新に保つ。v271 で Notion §0.1 から分離
- **CHANGELOG.md** = 現行 Phase の変更履歴詳細（過去分は CHANGELOG_PHASE*.md にアーカイブ・read-only）
- **TODO.md** = やること概要（🔥🟡🟢🔧🎮 で分類）
- **Notion 乗レコ全体像** (id 36171b458b638120b572fed38140e054) = 仕様の詳細・真実の源。§1.x 画面 / §2.x 裏側 / §2.6 データの流れ / §2.7 命名辞書 / §2.5 落とし穴 / §3 将来。§0.1 は STATUS.md へのリンクのみ
- ずれたらコードが真実
