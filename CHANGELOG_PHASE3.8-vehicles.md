# 乗レコ - 電車旅 更新履歴 (Phase 3.8 through_lines + GPS 位置づけ変更 + 車両形式 DB + 記録モーダル整理期 — v334〜v363 アーカイブ)

`CHANGELOG.md` から退避した Phase 3.8 through_lines / GPS 位置づけ変更 / 車両形式 DB / 記録モーダル整理 / 路線詳細モーダル / メモ車両 / CHANGELOG 行数チェック導入 (v334〜v363 相当, §184〜§213) のセッションログ。
他フェーズは:
- [CHANGELOG.md](CHANGELOG.md) — 現行 (v364〜)
...
- §213 v363: メモに train_name 列追加、「🚆 あずさ [E353系]」形式の表示
- §212 v362: メモ車両 cascade 整理 (`#m-car-model` を `__custom__` 選択時のみ表示)
- §211 v361: メモ車両選択を記録モーダル方式 (フル cascade) に統一
- §210 v360: メモにも車両形式 (migration + 記録/編集モーダル input + 一覧表示 + 検索)
- §209 v359: 路線タブ 📺 旅程 / 📸 メモ アイコン + 路線詳細モーダル
- §208 v358: 路線タブに乗車車両 上位 3 個を表示 + 車両形式検索
- §207 v357: 旅程タブに「🚆 車両」検索 input を追加 (substring 検索)
- §206 v356: 旅程編集モーダル: 特急等選択時に列車 dropdown を追加 (記録モーダルとの対称性向上)
- §205 v355: 旅程編集モーダル整理 (カテゴリ「普通」先頭 + 列車名 input を「普通」/「指定しない」で hide)
- §204 v354: 旅程カードに車両形式を表示 (普通=車両形式のみ / 特急=列車名+車両形式)
- §203 v353: 記録モーダル カテゴリ dropdown の並びを「普通」先頭に
- §202 v352: 記録モーダル 普通/特急ラジオを撤廃 → 既存カテゴリ dropdown 駆動に統一
- §201 v351: 記録モーダル 「(マニア向け)」文言削除 + 普通電車レーンに「✏️ 別形式を入力」自由記述 option
- §200 v350: 記録モーダル整理 (遅延入力を独立トグル化 + 普通/特急 ラジオで車両形式 UI を排他表示)
- §199 v349 (no deploy): ドキュメント整理 (CHANGELOG.md 5722 行 → 749 行に 4 アーカイブ分割 + Stop hook に行数チェック導入)
- §198 v348: 営業系統×車両形式 DB Phase 3 (記録モード UI: C' 案・マニアトグル + 区間→候補車両)
- §197 v347: 営業系統×車両形式 DB Phase 1 (Notion → JSON exporter + service_line_vehicles.json)
- §196 v346: 「GPS に変換」ボタン (retroactivelyVerifyTrip) を撤去
- §195 v345: GPS 記録の位置づけ変更 + 不正検知撤回 + バッジ中立化
- §194 v344: 「📝 後追い」バッジ + 「📌 記録」行を GPS 記録 (verified) 限定に
- §193 v343: Phase D3 第三セクター北陸・東北 + 福岡空港線 + 私鉄細支線 関東 (23 ペア / 46 ref)
- §192 v342: s0/s1 セグメント分割路線の sibling 機構 (案 B 採用)
- §191 v341: Phase D2 名古屋エリア through_lines 19 ペア (38 ref) 追加
- §190 v340: 山陽電鉄 through_lines 補完 (Phase C 漏れ 2 ペア)
- §189 v339: 山形/秋田 ミニ新幹線を独立系統として新設 + 東北新幹線と through_lines 接続
- §188 v338: through_lines 双方向化バグ修正 (v334 由来の片方向参照 8 件)
- §187 v337: Phase C 関西主要 27 直通ペア (54 ref) を through_lines に追加
- §186 v336: Phase B 関東主要 26 直通ペア (52 ref) を through_lines に追加
- §185 v335: 新幹線 3 直通ペアを through_lines に追加 (双方向)
- §184 v334: through_lines (直通系統) を本格運用化 (3 系統追加 + broken refs 修正 + UI)
