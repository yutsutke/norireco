...
`東京→拝島` のように **1 本の系統では繋がらない区間**を渡すと、エラーにする代わりに
「どこで乗り換えれば繋がるか」を計算して `routes` に入れて返す。本体
`js/07-record-mode.js` の `findTransferCandidates`（v365 / v366 直通優先 / v367 徒歩乗換）
の移植で、次の順に並ぶ。

1. `direct_through: true`（直通電車があるので実際には乗り換え不要）
2. 総駅数が少ない順

`walk_to` が付く候補は「その駅まで歩いて乗り換え」（新宿 → 西武新宿 など）。1 回乗換で
繋がらないときは 2 回乗換も探すが、探索量に上限を置いていて超えたらそこまでの結果を返す。
...
3. **接続できる人を自分だけに絞る（初回は推奨）**
   ...
   乗レコ アカウントに記録できる（他人のデータは RLS で触れない）。
...
```sh
npx wrangler secret delete ALLOWED_EMAILS   # 全開放
npx wrangler secret put ALLOWED_EMAILS      # また絞る（メールをカンマ区切りで）
```

開放しても各自が自分のアカウントに記録するだけで、他人のデータには触れない（v421 の RLS）。
1 ユーザーあたり **1 日 500 回・記録は 60 件**の上限があり、超えるとその人だけその日は
打ち止めになる（`wrangler.toml` の `DAILY_LIMIT` / `DAILY_WRITE_LIMIT` で調整可）。

ただし **Google アカウントを増やせばその数だけ枠が増える**ので、総量の保証にはならない。
総量が心配なら Cloudflare 側で使用量アラートを設定すること。
