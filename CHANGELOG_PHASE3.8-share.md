# 乗レコ - 電車旅 更新履歴 (Phase 3.8 シェア + Cloudflare 移行期 — v226〜v249 アーカイブ)

`CHANGELOG.md` から退避した Phase 3.8 シェア MVP + 完乗率統合 + 系統色 + Cloudflare 移行期 (v226〜v249 相当, §75〜§98) のセッションログ。
他フェーズは:
...
- §98 v249: GitHub Pages → Cloudflare Pages 移行 + 独自ドメイン norireco.app
- §97 v248: HTML onclick の window bridge 漏れ修正 (toggleRecordMode 他)
- §96 v247: 系統色カスタマイズの Supabase 同期 (デバイス間共有)
- §95 v246: 記録モード中の polyline click 抑制 + ESC で色モーダル閉じる
- §94 v245: 地図クリックで系統色を変更
- §93 v244: 駅マーカーも色 override に追従
- §92 v243: 系統色のユーザーカスタマイズ機能
- §91 v242: 同名駅の誤マッチ修正 (REGION_CENTER の緯度経度判定化)
- §90 v241: 9 地域の中央駅 10 駅を tier 7 で日本全国ビューから表示
- §89 v240: マイページ「公式」表記を「GPS 記録」に統一
- §88 v239: ヘッダ/マップオーバーレイの系統数オーバーカウント修正
- §87 v238: ヘッダ完乗率とマイページ完乗率の数字ズレ修正
- §86 v237: OGP 日本地図を Natural Earth ベース 47 都道府県境界に置換
- §85 v236: シェア機能 MVP (OGP 画像生成 Canvas)
- §84 v235: 完乗率の集計方式を「ユニーク駅単位」に統一
- §83 v234: 静的デモデータ `RIDDEN_SEGS_STATIC` を撤去
- §82 v233: 未ログイン時の Supabase 同期で他人の trip / キャラ獲得が漏れていたのを修正
- §81 v232: 駅ドットとパイチャートの出現タイミングを統一
- §80 v231: 駅ランク tier テーブルを圧縮 (都内クラッタ抑制)
- §79 v230: 地図 LOD から首都圏 bbox 分岐を撤去、駅ランク 1 本化
- §78 v229: 達成率/系統数バッジと mypage キャッシュも purge
- §77 v228: ログアウト時に地図の乗車データをローカルから purge
- §76 v227: 旅程カードのボタンラベル「メモ編集」→「編集」
- §75 v226: 旅程カード「✏️ 編集」を区間・乗車時刻・列車種別まで拡張
