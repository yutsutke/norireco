...
- [ ] **Notion 子ページの中抜けバージョン補完（v132〜v180）**
  - Notion §0 + 子ページの体系は v130〜v131 のコード分割期に作られた。それ以降のセッションで §0 と命名辞書だけ更新して、画面/機能の子ページに反映が漏れた可能性が高い
  - **まず棚卸し**: 全子ページ (1.1〜1.5 / 2.1〜2.7 / 1.3 記録モードの関連サブページ「不正検知」「現在地・GPS」「列車・車両形式」など) の「バージョン履歴」セクションを fetch して最新版数を一覧化
  - 既に確認済みの抜け（参考）:
    - **1.2 🗾 地図画面**: v177 マップ表示モード切替（部分乗車系統 bg/solid 選択描画）
    - **1.3 📝 記録モード**: v135〜v137 ログイン / v175 用語統一 / v178 乗車日時編集 / v179 記憶の精度 5 段階 / v180 🕒 時刻行連動
    - **1.4 👤 マイページ**: v178 「📝 後追い」バッジ + 「📌 記録: ...」 / v179 精度バッジ（〜 紫・❓ 紫）と filterTripsByDate の unknown 除外
    - **1.5 🔐 ログイン**: v135〜v137 Magic Link + Google OAuth 実装の詳細（v138-v157 のマイページから派生した変更）
    - **2.1 🗂 データアーキテクチャ**: v179 date_precision 拡張 / v181 notes・delay_minutes（schema 未拡張）
    - **2.2 🗄 Supabase**: v135〜v137 認証カラム / v97 認証フィールド / v123 train 系 4 カラム の経緯整理
    - **2.4 📁 コード構成**: js/01〜13 のファイル拡張（v131〜v138 で 10→13 分割）
    - **2.5 ⚠️ 落とし穴**: tripForSupabase workaround（schema 未拡張時の送信除外 + localStorage merge）、TZ バグ（localDateStr ヘルパ、v133 教訓）
    - **2.6 🔀 データの流れ**: 認証グラデーション v89 / GPS 認証フィールド v97 / 列車スキーマ v123 / 不正検知 v132 / 期間フィルタ v157
  - 作業方法: CHANGELOG.md §17〜§29 (v109〜v180) を該当子ページに転記。命名辞書 2.7 は今セッションでカバー済み
  - スコープが大きいので 1 セッションで終わらない可能性あり。子ページごとに分けても良い
