...
**現在の SW**: v173 / **キャラ**: 7体（八王子3・立川3・小宮1）
...
**直近の作業**: データ補修（西武池袋線駅順・箱根登山系 operator_id）（v173、Phase 3.8）
...
- [ ] **期間指定機能の拡充 + 統計連動 (タイムマシン廃止)**
  - 既存「🕰 タイムマシン」サブタブは複雑だったので廃止し、シンプルなマップ期間指定に統合
  - 期間指定モード:
    - **〜YYYY/MM** (最初から指定月まで) — 過去の達成状況を年月単位で振り返り
    - **YYYY/MM〜YYYY/MM** (期間)
    - **今日まで** (デフォルト)
  - 統計タブ・路線一覧・マイページの完乗率も期間指定に追随
  - 過去状態の地図・パイチャート・達成率が同じ期間軸で連動
  - タイムマシン UI (4 つ目のサブタブ) は撤去 → サブタブを 3 つに

- [ ] **マップ表示モード切替（乗車/未乗車フィルタ）**
  - マップ上にトグル: 「両方」「乗車路線のみ」「未乗車路線のみ」
  - 「未乗車のみ」モードで次に乗るべき路線が一目で分かる（旅行計画用途）
  - 「乗車のみ」モードで自分の足跡をスッキリ見られる（達成感）
  - 期間指定機能 (上記) と組み合わせ可能にする
...
- [ ] **後追い記録モード（時間を空けてから登録）**
  - しばらく時間を空けてから乗車記録を登録したいケース対応
  - 入力オプション:
    - 「何時の電車に乗り、何時に着いたか」を手動で時刻指定
    - 列車遅延 (○○分遅れ) のメモ
    - 駅の設備メモ（トイレあり、混雑度、改札位置 等）
    - 当該乗車のエピソード・所感メモ
  - 振り返り用途と次回の予定立て用途で両方便利
  - GPS フローの「verified」を取らない代わりに、メモが充実する分 manual の価値が出る
  - 既存の `trip` レコードに `notes` / `delay_minutes` / `station_notes[]` 等を追加
