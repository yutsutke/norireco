...
**現在の SW**: v157 / **キャラ**: 7体（八王子3・立川3・小宮1）
...
**マイページ**: 4 サブタブ (統計 / 旅程 / 路線 / タイムマシン)、詳細統計 16 種、タイムマシンで全タブ過去化

---

## 🔥 最優先（プロダクトとして欠けている）

- [x] ~~**Supabase 認証＋ログインページ**~~ → **✅ v135〜v137 完成**
  - Magic Link + Google OAuth (Supabase Auth)
  - `user_id UUID` を 3 テーブル (trips/character_grants/memos) に追加
  - 初回ログイン時に NULL レコードを自分の uid に backfill
  - ヘッダにアバター/ログインボタン + モーダル UI

- [x] ~~**マイページ**~~ → **✅ v138〜v157 完成**
  - 4 サブタブ: 📊 統計 / 🚃 旅程 / 📋 路線 / 🕰 タイムマシン
  - 🟢 公式完乗率 / ⚪ 全記録完乗率 (ユニーク駅ベース、常時表示)
  - 詳細統計 16 種 (累計駅数推移・都道府県別・PR・路線/駅タイムライン・ヒートマップ・未踏領域・車両形式コレクション・会社別 進捗 etc)
  - GPS 後追い認証 (📍 半径 500m 以内で verified=true 昇格)
  - 旅程削除 / 期間フィルタ・認証フィルタ・種別フィルタ
  - タイムマシン: 過去任意日付の全タブ表示 (グローバル date filter 連動)
  - IC履歴認証は技術的に実現不可と判明 → 削除 (将来 OCR で代替案検討)
...
- [ ] **記録モード認証グラデーション 続き（Step b/c）**
  ...
  - Step b: ⚪🟢⭐ バッジを trip 一覧・モーダルに表示 ← **マイページで完了**、stop_type 反映は未着手
  - ~~Step c: 後追い登録 UI（既存 trip を後から認証昇格）~~ → **✅ v138 マイページの GPS 後追い認証で完成**
  - 5分滞在 (✅) は当面スキップ、IC カード取込はスキップ確定 (技術的に不可)
...
- [ ] **記録モード名称のリネーム**
  - 「区間記録モード」→「**手動記録**」
  - 「📍 今ここ！」系の GPS 取得 →「**GPS 記録**」として用語統一
  - UI 文言・モーダル・ヘルプ・トースト・コード内コメントを横断置換
...
- [ ] **同一会社内でも別系統なら乗り換え判定にする**
  - 例: 西武池袋線 ↔ 西武秩父線 は同じ西武鉄道だが系統が違うので乗り換え扱いにすべき
  - 現状の乗り換え判定ロジックを「会社ベース」から「系統ベース」に見直す
  - 影響範囲: 区間記録の経路推定・所要時間チェック (`js/11-fraud-detection.js`)・統計の乗換回数

- [ ] **駅順データのバグ修正**
  - 西武池袋線（飯能・東飯能・高麗）の並びがおかしい → `service_lines_master.json` の stations 配列を実線形通りに修正
  - 他系統も同様の並び不整合がないか棚卸し

- [ ] **箱根ケーブルカーの駅情報を最新化**
  - 駅名・駅順・運行情報を最新の公式情報に更新
  - 該当ファイル: `service_lines_master.json` の箱根ケーブル系統
