...
**現在の SW**: v203 / **キャラ**: 7体（八王子3・立川3・小宮1）
...
**直近の作業**: **ES Modules パイロット (案 β) stage 2 拡張** — 11-fraud-detection.js (state 0) を `<script type="module">` 化、`fraudAssessTrip` / `fraudIsDowngraded` を window 公開（v203）/ 12-auth.js を type=module 化 + 4 関数 window 公開（v202）/ stage 1 全 7 ドメイン (v195〜v201、累計 46 state)
