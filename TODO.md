...
- [ ] **一括記録 (まとめて記録)** (Notion §1.3 設計確定 / B カテゴリ完結 v392-v399 / **A 段階 A-1〜A-8 完結 v400-v406** = 全機能 + 区間ピッカー / **v407 で旧 `noritetsu-log.html` 削除完了** = 受け皿として完全置換、**写真添付 ✅ v434** / **① 環状線 ✅ v435** = v422 で既に解消済みと実機計測で確定 (山手線 30/30・大阪環状線 19/19)、偽の注意書きのみ撤去 / 残: ③ 複数 segment = 設計上スキップ推奨 (1 系統 1 行モデル / 手動記録 07 で代替) → ユスケ要否確認)
  - **動機 2 層**:
