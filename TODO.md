...
- [ ] **直通系統の追加**（service_lines_master.json の `through_lines`）
  ...
  - 残 (Phase D 散発):
    - 名古屋エリア (名鉄/JR東海/名古屋市営地下鉄)
    - 北陸新幹線↔IRいしかわ/あいの風とやま/ハピライン (第三セクター転換系)
    - 山陽電鉄本線が service_lines_master 未追加 → データ追加してから神戸高速線/阪神に through 接続
    - s0/s1 セグメント分割路線への対応改善 (中央線_s1 / けいはんな_s1 / 北大阪急行_s1)

- [ ] **山形/秋田ミニ新幹線の独立系統新設**
  - 福島〜新庄 (山形新幹線つばさ) / 盛岡〜秋田 (秋田新幹線こまち) を v334 の青梅線方式で `yamagata_shinkansen` / `akita_shinkansen` として手動キュレーション系統新設
  - 新設後、東北新幹線との through_lines を双方向で接続
