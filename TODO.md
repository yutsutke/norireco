...
- [ ] **シェア機能 — MVP 以降の残り (v236 で OGP 画像生成 MVP は完成)**
  ...
  - **シェア機能 残り** (S-1〜S-3 で MVP 完結。以降は磨き込み + 拡散強化):
    ...
    - 🟡 v438 (実装済・**SQL 要 Run**): **admin シェア計測 横断ビュー** — 運営が全シェアの漏斗 (view→click→signup) を俯瞰。admin タブに 🚫 BAN管理 / 📊 シェア計測 のビュー切替 + サマリ (Σ + CTR/登録率) + シェア別ランキング (所有者 email 付き)。RPC `admin_list_share_metrics` (SECURITY DEFINER + is_admin ゲート + auth.users join、既存 admin 関数ファミリと同形)。**⚠️ `supabase/migrations/v438_admin_share_metrics.sql` を Run 要**。CHANGELOG §285
