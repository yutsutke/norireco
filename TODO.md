...
- [ ] **シェア機能 — MVP 以降の残り (v236 で OGP 画像生成 MVP は完成)**
  ...
  - **シェア機能 残り** (S-1〜S-3 で MVP 完結。以降は磨き込み + 拡散強化):
    ...
    - ✅ v439: **シェア計測/一覧画面を 1 枚にスクショ保存するボタン** — admin シェア計測 と マイページ🔗シェア一覧 に「📷 スクショ保存」(html2canvas を cdnjs から lazy load、要素まるごと=見切れる縦長も PNG 化 → ダウンロード)。共通ヘルパーは 14-share-ogp.js (`window.NORIRECO.share.captureElementToPng`)、admin は window 経由。CHANGELOG §286。注: マイページのサムネは R2 の CORS 次第で写らない可能性 (要本番確認)
