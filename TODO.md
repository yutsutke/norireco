...
**直近の作業**: → [`CHANGELOG.md`](./CHANGELOG.md) の最新セクション (v457 時点で §296-§304 = iOS 対応 →「堀を作らない」→ MCP サーバ が現行) を参照。各 commit ごとの背景・設計判断・失敗教訓まで含む。要約だけほしいときは git log でも可:
```
git log --oneline -20
```
...
- [ ] **MCP サーバ（AI チャットから乗車記録）— 稼働中・拡張の余地あり**
  ...
  - [ ] **マジックリンクでのログイン対応** — いま Google ログインのみ。**問い合わせが多ければ対応する程度で考える**（ユスケ判断 2026-08-29）
  - ✅ **記録の削除ツール** `delete_trip` (2026-08-29): trip_id 指定のみ・写真も R2 から掃除
  - ✅ **完乗率の取得ツール** `get_completion` (2026-08-29): 本体 13a-stats と同じ数え方。line 指定で 1 系統の進捗、since/until で期間指定
...
- [ ] **シェア機能 — MVP 以降の残り (v236 で OGP 画像生成 MVP は完成)**
  ...
  - **シェア機能 残り** (S-1〜S-3 で MVP 完結。以降は磨き込み + 拡散強化):
    ...
    - ✅ v436: **受け側ページ強化 (③) + view/click 計測 (④)** — MVP サブ項目が全 ✅ で空だった本 TODO を拡散バリューチェーンで再定義し ③+④ を着手 (ユスケ確定 2026-05-31)。`/share/<id>` に CTA 文脈化 (kind 別) + 「乗レコでできること」価値訴求 3 項目、view (bot UA 除外) + CTA click (`/share/<id>/go` リダイレクト Function 経由) を `SECURITY DEFINER` RPC `bump_share_metric` (anon EXECUTE) で計測 (表示は admin 限定・v440 でユーザー非表示に)。CTA に `?ref=s_<id>` 付与 (Phase 2 attribution の土台)。`v436_share_metrics.sql` Applied (2026-05-31) + 実機確認済 (view=4/click=1)。`functions/` は main push で自動反映。CHANGELOG §283
    - ✅ v437 (SQL Applied 2026-05-31 + 実機 E2E 確認済): **登録転換 attribution (Phase 2)** — 「シェアが分水嶺」(5大原則④) の本命指標。新規アカウントをシェア経由登録 → `norireco_share_referrals` に 1 行 + `signup_count` +1 を実機確認。`captureShareReferral` が起動時に `?ref=s_<id>` を localStorage 退避 (OAuth/Magic Link リダイレクトで query 消失するため)、`maybeRecordShareReferral` が初回ログイン確定時かつ新規アカウント (`user.created_at` 直近 24h) のとき RPC `record_share_referral` を叩く。`norireco_share_referrals` (user_id PK = once-per-user) が真実の源・`shares.signup_count` が派生キャッシュ。自己シェア/revoked 除外、EXECUTE authenticated のみ。✨登録 の表示は admin 限定 (v440 でユーザー非表示に)。`v437_share_referral.sql` Applied (2026-05-31)。CHANGELOG §284
    - ✅ v438 (SQL Applied 2026-05-31 + 実機確認済): **admin シェア計測 横断ビュー** — 運営が全シェアの漏斗 (view→click→signup) を俯瞰。admin タブに 🚫 BAN管理 / 📊 シェア計測 のビュー切替 + サマリ (Σ + CTR/登録率) + シェア別ランキング (所有者 email 付き)。RPC `admin_list_share_metrics` (SECURITY DEFINER + is_admin ゲート + auth.users join、既存 admin 関数ファミリと同形)。実機で 👁7→🚃2(CTR28.6%)→✨1(登録率50%) 確認。CHANGELOG §285
    - ✅ v439 → v440: **画面を 1 枚にスクショ保存するボタン (admin 限定)** — html2canvas を cdnjs から lazy load、要素まるごと=見切れる縦長も PNG 化 → ダウンロード。共通ヘルパーは 14-share-ogp.js (`window.NORIRECO.share.captureElementToPng`)。**v440 でユーザー向けマイページの計測表示 (👁/🚃/✨) とスクショボタンを撤去 → admin シェア計測ビューだけに集約** (ユスケ判断「お客さんには不要・管理者だけで十分」)。計測カウント自体・DB 列・admin 横断ビューは不変。CHANGELOG §286/§287
    - ✅ v443: シェア画像に期間チップ — 完乗率マップのシェア (openShareModal) に「この期間[地図フィルタ準拠デフォルト] / 今年 / 全期間 / 任意期間」チップ。選んだ期間で完乗率 (`computeCompletionStats` に切り出し) + 地図ポリラインを再計算して画像再生成。期間変更はシェア画像のみ (グローバル `_tripDateFilter` 不変)。個別 trip シェアでは期間チップ非表示。CHANGELOG §290
    - **残り (将来)**: created_at 24h ゲートの精緻化は実運用データを見てから (v437)
...
- [ ] **活性化（アクティベーション）強化 — 使う人をどう増やすか** (2026-06-03 着手、ユスケ「宣伝広告・どう増やすか」議論から)
  - **方針**: ニッチ無料アプリで有料広告は CAC が見合わない。5大原則④「早期βで学習 → 本リリースで拡散」に従い、**拡散の前にバケツの穴 (活性化/継続) を塞ぐ**。ファネル診断で「完乗率/完乗達成/streak は計算済みなのに📊統計深部の受け身の数字止まり」= 能動的な瞬間が無いのが核心
  - ✅ v444: **初回の達成演出** — 一括保存直後に「全国○○駅 制覇！+完乗率+今回塗った差分+新規完乗系統」を全画面オーバーレイで祝い → シェア (原則④) 導線。新規 `22-celebrate.js`。検証で `_mypageCache` がオンボーディング経路で null = 演出/シェア空集計の穴を発見 → localStorage hydrate で修正。CHANGELOG §291
  - ✅ v447: **記録モーダルの入力を前回値で初期化** (記録の摩擦低減 = 継続率向上)。記憶の精度を localStorage にグローバル記憶 + 列車・車両形式カスケードを系統 (lineId) ごとに記憶 (車両形式は路線固有なので同じ路線にのみ復元 — AskUserQuestion 判断)。「日付のみでつけ始めたら毎回まっさらに戻りそればかりになる」摩擦の解消。CHANGELOG §294
  - **残り (次の候補・本タスクの議論で整理)**:
    - 🟡 **戻る理由 = ネクスト目標の常時提示** (「あと○駅で△△線 完乗！」を地図/駅に表示)。Push 通知が無いので「アプリ内で次の near-term ゴールを与えて自走させる」形。完乗・streak データは既にあり surface するだけ = 活性化ファネル④ の本丸
    - 🟡 **手動記録 (07 saveMultiSegmentTrip) でも完乗到達時に演出** (現状は一括記録のみトリガ)
...
- [ ] **#7 駅 ID name 照合の物理撤去（駅 ID 体系 完全版・今はやらない）** ← 駅 ID 体系 Phase 1〜3 完了 (v290〜v329) + Phase 2 クローズ (v422) の残務
  ...
  - ✅ 解消 (v461): `tripsToSegs` が `seg.from_id` / `to_id` を落としていたため 04b `rebuild` の id 優先分岐が一度も通っていなかった件を修正（v460 調査で発覚）。全 638 系統 10,501 駅で「id 引き == name 引き」「id が null の駅 0 件」、擬似旅程 1,914 件で `slRiddenSt` 差分 0 を機械検証済（挙動不変）。詳細 → CHANGELOG §308
  - これにより ① の前提が変わった: RIDDEN_SEGS に駅 id が乗るようになったので、**残る name 照合サイトは backfill を待たずに id 優先化できる**（name フォールバックを残したまま）。次に着手するなら `02b-service-lines-builder.globalStats` と `14-share-ogp.buildSegmentPolylines` の 2 つが手近（どちらも `sl.stations.findIndex(s => s.name === seg.from)` 1 行ずつ）。backfill が要るのは ③ の「name 照合の物理撤去」だけ
