...
- [ ] **Notion 子ページの中抜けバージョン補完（v173〜v180）**
  - 過去セッションで §0 と命名辞書だけ更新して、画面/機能の子ページに追記漏れがあった
  - **1.2 🗾 地図画面**: v177 マップ表示モード切替 (両方/乗車のみ/未乗車のみ) — `window._mapDisplayMode`、部分乗車系統の bg/solid 選択描画
  - **1.3 📝 記録モード**: v175 用語統一（手動記録 / GPS 記録）/ v178 乗車日時編集 + 「📝 後追い」可視化 / v179 記憶の精度 5 段階（`date_precision = minute/day/month/year/unknown`）/ v180 確認モーダル🕒 時刻行を記録種別・精度連動表示
  - **1.4 👤 マイページ**: v178 「📝 後追い」バッジと「📌 記録: ...」行 / v179 精度バッジ（`〜` 紫・`❓` 紫）と `filterTripsByDate` の unknown 除外
  - **2.5 ⚠️ 落とし穴**: tripForSupabase workaround の落とし穴（schema 未拡張時の送信除外 + localStorage merge）、TZ バグ (`localDateStr()` ヘルパ) の補強
  - 作業方法: CHANGELOG.md §22〜§29 (v173〜v180) を子ページの「バージョン履歴」セクションに追記。命名辞書 2.7 は今セッションでカバー済み
