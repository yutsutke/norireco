# 乗レコ - 電車旅 更新履歴 (Phase 3.8 乗換候補 + 徒歩乗換 + 系統別車両形式 + 旅程編集 per-seg cascade 期 — v364〜v391 アーカイブ)

`CHANGELOG.md` から退避した Phase 3.8 後半 (v364〜v391 相当, §214〜§241) のセッションログ。主軸テーマ: 乗換候補自動提案 (1 hop / 2 hop fallback / 直通あり) + 徒歩乗換グループ DB + 系統別車両形式 (T.selectedXxxBySl Map + chip 切替) + 記録モード「区間 → カスケード」順 per-seg 化 + 旅程編集モーダルも per-seg cascade 対応 + サブエージェント方針実装。

他フェーズは:
- [CHANGELOG.md](CHANGELOG.md) — 現行 (v392〜, trip 詳細エディタ抽出)
...
- §241 v391 (no deploy): サブエージェント `js-syntax-guard` 配置 (Notion 「🤖 サブエージェント方針」実装)
- §240 v390: 手動記録保存後、駅アクションシートのカウントが即座に反映 (`_mypageCache` 同期 + `refreshTripListIfOpen` 呼出)
- §239 v389: 旅程削除後に駅アクションシートの trip カードも即座に消える (`NORIRECO.stationActions.refreshTripListIfOpen` 公開)
- §238 v388: 旅程削除後にマップへ即時反映 (リロード不要に) — `deleteTripFromMypage` 末尾で `applyDateFilter()` → drawLines 一気通貫
- §237 v387: 駅フィルタ「乗車のみ」が実質効かない問題を解消 (`slStopType` 集計を `slRiddenSt` と同ループに統合 + resolve fallback 共有)
- §236 v386: ロゴクリックでマップ画面トップへ遷移 (`<a href="noritetsu-map.html">` 化)
- §235 v385: 確認モーダルに「乗換あり旅程の時刻仕様」注記を追加 (`openRecConfirm` の multi-segment branch、青系 info-box)
- §234 v384: 旅程編集モーダル: cat 切替時の DOM 値残り問題を解消 (`applyTripEditSegCategoryVisibility` の local / other 分岐に value クリア追加)
- §233 v383: 旅程編集モーダルも per-seg フル cascade 対応 (種別 select / 列車 dropdown / 列車名手入力 / 車両形式 dropdown + 手入力 を seg row に生成)
- §232 v382: パイチャート (divIcon) のみ alighted 倍率を 2.5 → 1.5 に調整
- §231 v381: 乗降した駅 (alighted) のドットを目立たせるよう倍に拡大
- §230 v380: 旅程編集モーダルも per-segment cascade (カテゴリ + 列車名 + 車両形式) 対応 (集約ルール: 全 seg 一致なら値 / 不一致なら null)
- §229 v379: v378 hotfix: 古い trip (v374 以前) の特急が「列車制覇」に出ない (segments に train_id が無い旅程の trip.train_id fallback)
- §228 v378: 「列車制覇」統計を segments 走査に対応
- §227 v377: 旅程カード表示を per-segment 列車・車両に対応 (segments[].car_model の unique 値を joining)
- §226 v376: v375 hotfix: `window.populateSlVehiclePicker` 未公開
- §225 v375: 記録モード「区間 → カスケード」順に再編 + 完全 per-segment 化 (`segments[].train_category / train_id / train_name / car_model` 全部 per-seg 化、各 handler が `T.selectedXxxBySl[activeChipSlId]` に同期書き込み)
- §224 v374: 記録モード「特急選択時も系統別車両形式」対応 + state 分離 (`T.selectedCarModel` cascade 専用 + `T.selectedCarModelBySl` Map SL chip 専用)
- §223 v373: 旅程編集モーダルも per-segment 車両形式編集に対応
- §222 v372: v371 hotfix: `const T` 二重宣言で画面真っ黒 (ESM 構文チェック教訓: `node --check --input-type=module < file` で関数内 const 重複検出)
- §221 v371: 記録モードに「系統別車両形式」対応 (`T.selectedCarModelBySl` Map + chip 切替時 dropdown 値の save/restore)
- §220 v370: 路線フィルタを `lineId × SERVICE_LINES` 逆引き方式に整理 (駅検索 v317 と同形、O(L+N))
- §219 v369: マイページ旅程タブに「🛤 路線」substring フィルタ追加
- §218 v368: 駅アクションシート「この駅を含む旅程 (タップで読み込み)」のカッコ書きを削除 (アクションシート文脈で自明)
- §217 v367: 徒歩乗換グループ DB + 記録モード walk segment / walk fallback 候補 (243 グループ / 553 駅自動抽出、Union-Find transitive closure、`walk_transfers_overrides.json` で手動修正可)
- §216 v366: 乗換候補: 直通系統「直通あり」バッジ + 2 hop fallback (1 hop ゼロのとき 2 hop top 3、駅→系統索引キャッシュで ~400ms)
- §215 v365: 記録モード: 別系統 2 駅選択時に乗換駅候補を自動提案 (1 hop top 5、id ベース + 総駅数昇順、chip タップで R.selection 挿入 + lineA/lineB pre-select)
- §214 v364 (no deploy): CHANGELOG 行数チェックを Stop hook → セッション末手続きに移管 (警告頻度が過剰だったため)
