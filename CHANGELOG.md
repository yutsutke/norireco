...
## 308. v461 — 🔧 `tripsToSegs` が駅 id を捨てていた — v422 の「id 優先」分岐を実際に通す

**カテゴリ**: A（v460 の調査で見つけた残課題をユスケ指示で回収）

### 何が起きていたか

v310 で `segments` に `from_id` / `to_id` を入れ、v422 で `04b-ride-record.rebuild()` を
「id 優先・name フォールバック」にした。が、`js/05-supabase-data.js` の `tripsToSegs` が

```js
segs.push({ lineId: seg.lineId, from: seg.from, to: seg.to });   // ← id を捨てている
```

と id を落としていたため、**`rebuild()` の id 分岐は一度も通っていなかった**（常に name 側）。
コメントには「id 優先」と書いてあるのに実行されない、という典型的なドキュメントとコードのずれ。

さらに悪いのは **同じデータが 2 経路で違う扱いになっていた**こと:

| 経路 | RIDDEN_SEGS に入る形 | 照合 |
|---|---|---|
| 保存直後（`07-record-mode:1413` / `21-bulk-record:917` が `trip.segments` をそのまま push） | id 入り | **id 経路** |
| リロード後・期間フィルタ・Supabase 同期（`tripsToSegs` 経由） | id 無し | name 経路 |

「保存した直後は正しいのにリロードすると挙動が変わる」類の不具合が、いつ出てもおかしくない状態だった。

### 直し方

`tripsToSegs` に `from_id` / `to_id` を持ち回らせるだけ（1 行）。これで両経路が揃う。

### 安全性の機械検証

「id 経路に切り替わって結果が変わらないか」を、推測ではなく実データで確かめた。

**① 系統内で id 引きと name 引きが着地点を違えないか**（全 638 系統 10,501 駅エントリ）

- id が `null` の駅: **0 件**（= id 引きは必ず試される）
- 同一系統内の駅名重複: 1 件（富山地鉄本線の 稲荷町 が idx 16/17 に重複。ただし id も同じ
  `s_06897` なので id 引き・name 引きとも idx 16 に着地）
- **id 引き ≠ name 引き となる駅: 0 件**

**② `slRiddenSt` の全系統差分**（全 638 系統 ×3 区間 = 1,914 件の擬似旅程で旧実装と比較）

- id 分岐が発火した区間: **1,914 / 1,914**（旧実装では 0）
- `slRiddenSt` が食い違った系統: **0 / 638** → 挙動不変

**③ v460 の報告ケースの通し再現** — モノレール 12/19 駅 / 小田急多摩線 2/8 駅。v460 と完全一致。

### 効くのはいつか

現行マスタでは挙動は変わらない。効くのは **駅名が改称されたとき**で、旧名で保存済みの trip が
地図から静かに消えるのを id 側が受け止める。今までは name 一本だったので、改称のたびに過去の
記録が無言で欠けていく片道リスクを抱えていた。

### 今回も触っていないもの

`02b-service-lines-builder.globalStats` と `14-share-ogp.buildSegmentPolylines` は
まだ name 照合のまま。RIDDEN_SEGS に id が乗るようになったので **backfill 無しで id 優先化できる**
ようになったが、今回のスコープ外（→ TODO.md 布石 #7）。

---

## 307. v460 — 🐛 「乗ったのに点線が消えない」— 未乗車の点線を系統全体から未乗の駅間だけに変える

**カテゴリ**: A（ユスケ報告 2026-09-12「mcp で旅程を保存。多摩モノレール 立川北→多摩センター、小田急線で多摩センター→唐木田を乗ったのに、未訪問で点線が消えない。訪問でやると実線は出る」）

### 症状と、最初に疑って外したもの

MCP（AI チャット）から旅程を保存したのに、地図の点線が乗った区間に残ったまま。
報告文の「mcp で保存した」に引っ張られて **MCP 側の保存内容の疑い**（駅 id がずれている /
segments の形が本体と違う / 駅名の食い違い）から入ったが、これは全部シロだった。

- `mcp/src/lines.js` の解決結果を実データで再現 → 小田急多摩線は「多摩センター」入力でも
  正しく **小田急多摩センター (s_03959)** に解決される（本体の駅名と一致）
- `js/02b-service-lines-builder.js` の SERVICE_LINES 構築 → `js/05-supabase-data.js` の
  `tripsToSegs` → `js/04b-ride-record.js` の `rebuild()` まで Node で通しで再現したところ、
  `slRiddenSt` は正しく **モノレール 12 駅 / 小田急多摩線 2 駅** が入る

つまり保存も集計も正常。**壊れていたのは描画だけ**だった。

### 本当の原因: 点線を系統全体に 1 本引いていた

決め手はスクリーンショットの拡大。高幡不動・多摩動物公園・唐木田に **駅ドットが無い**のに
小田急永山・南橋本・上溝には有る。`drawStationsLayer` は `_mapDisplayMode === 'unridden'` の
とき乗車済み駅を skip するので、これは「乗車済みと判定されている」証拠 = 集計は生きている。
同時に、ユスケの言う「未訪問 / 訪問」は **駅フィルタのチップ（●降車 / ◎乗車 / ○通過 / □未訪問）**
のことで、□未訪問だけ ON = `_mapDisplayMode = 'unridden'` だと分かった。

`drawServiceLineBase` の乗車済みブランチは、背景の点線を **系統の全駅を通る 1 本**として
引いていた（v186 の駅フィルタ導入以来）。

- `both`（全チップ ON）: 点線の上に solid run が載るので、乗った区間は実線に見える → 気付けない
- `unridden`（□未訪問のみ）: solid run が抑制されるので、**乗ったばかりの区間が点線のまま残る**

「点線が消えない」も「訪問でやると実線は出る」も、これ 1 つで説明がつく。

### 直し方: 「駅」ではなく「駅間」で塗り分ける

乗車判定は駅単位（`slRiddenSt` = 駅 id Set）だが、線を塗るのは駅間なので、
**両端の駅がどちらも乗車済みの駅間だけ**を「乗った線路」とみなす（`unriddenEdgeRuns`）。
点線はそれ以外の駅間の連続ランごとに引く（`drawSlUnriddenRun`）。

この「両端とも乗車済みか」で判定するのが要で、乗車駅 → 隣の未乗車駅 の駅間（たとえば
立川北 → 高松）は乗っていない線路なので点線側に残る。結果として

- `both` の見た目は **従来と完全に同じ**（solid が覆っていた点線が無くなるだけ。solid は
  weight 3.5 / 点線は 2.2 なので元から見えていない）
- `unridden` では乗った駅間の点線が消える ← 今回の修正点
- 駅間の被覆に穴が空かない（点線 run と solid run で全駅間をちょうど 1 回ずつ覆う）

環状線は駅間 n-1（最後の駅 → 最初の駅）も数え、latlngs 生成を `% n` で回して wrap をまたぐ
ランもそのまま 1 本で引ける。`stations[0]` をまたぐと点線が 2 本に分かれることがあるが、
両者は `stations[0]` を共有するので隙間は出ない（dash の位相が切り替わるだけ）。

### 検証

実データ（`service_lines_master.json` + `lines-p*.json` + `merged_stations.json`）で
`unriddenEdgeRuns` を直接評価:

| ケース | 結果 |
|---|---|
| 小田急多摩線 唐木田+小田急多摩センター 乗車 | 点線は 小田急多摩センター→新百合ヶ丘 のみ ✅ |
| モノレール 多摩センター〜立川北 乗車 | 点線は 立川北→上北台 のみ ✅ |
| 完乗 | 点線なし ✅ |
| 飛び石（1 駅だけ乗車） | 両隣の駅間は未乗なので全区間点線のまま ✅（線が消えない） |
| 穴あき（中間だけ未乗） | 点線は穴の駅間だけ ✅ |
| 山手線 wrap 跨ぎ | 全 30 駅間を点線 2 本 + solid で過不足なく被覆 ✅ |

### 失敗教訓

- **報告文の「どこで起きたか」に引っ張られない。** 「mcp で保存したのに」は「mcp が原因」
  ではなかった。保存 → 集計 → 描画のどこで落ちているかを、まず通しで再現して切り分ける
- **スクリーンショットは拡大して読む。** 「駅ドットが無い」は肉眼サイズでは見落とす情報で、
  ここに「集計は生きている / 表示モードが unridden」という答えが両方写っていた
- ユスケの語彙は UI のラベルであることが多い。「未訪問 / 訪問」は駅フィルタのチップ名

### 残課題（今回は触っていない）

`js/05-supabase-data.js` の `tripsToSegs` が `seg.from_id` / `to_id` を落としている。
v422 で `rebuild()` を「id 優先・name フォールバック」にしたが、RIDDEN_SEGS に id が
乗らないので **id 経路は実際には一度も通っていない**（常に name フォールバック）。
1 系統の中に同名駅は無いので今のところ実害は無く、今回の症状とも無関係。→ TODO.md

---

## 306. v459 — 🔧 鶴見線の支線を切り出して地図の線の交差を直す + 検索タイトルに MCP 対応を出す

**カテゴリ**: A（ユスケ「ここらへんつながりがおかしいですね」+「検索タイトルにも mcp対応 を表示したい」2026-08-29）

### 鶴見線が「行って戻る」線になっていた

浜川崎〜扇町あたりで系統の線が交差していると指摘を受けて調査。`service_lines_master.json` の
鶴見線が、**支線の駅を本線の配列に混ぜて 1 本で持っていた**のが原因:

```
鶴見 国道 鶴見小野 弁天橋 [海芝浦 新芝浦] 浅野 安善 武蔵白石 [大川] 扇町 昭和 浜川崎
```

地図は営業系統の駅を配列順に直線で結ぶので、①支線へ寄り道して戻る線 ②末尾の並び違い
（正しくは 武蔵白石 → **浜川崎 → 昭和 → 扇町**）が、そのまま交差した線として出ていた。

リポジトリには既に「分岐は親 id + `_bN` の別系統として持つ」規約があり（阪和線羽衣支線 /
京葉線二俣支線 / 成田線空港支線 など 19 本）、**鶴見線だけそれをやっていなかった**。同じ形に揃えた:

| 系統 | 駅 | 総延長 |
|---|---|---|
| 鶴見線 | 鶴見 国道 鶴見小野 弁天橋 浅野 安善 武蔵白石 浜川崎 昭和 扇町 (10) | 6.6km |
| 鶴見線(海芝浦支線) `_b1` | 浅野 新芝浦 海芝浦 (3) | 1.4km |
| 鶴見線(大川支線) `_b2` | 武蔵白石 大川 (2) | 0.8km |

並びは merged_stations の緯度経度で検証（本線 6.6km は実際の鶴見線 7.0km とほぼ一致、
各駅間 0.5〜1.1km で飛びが無い）。作業は `tools/fix_tsurumi_branches.js` に冪等スクリプトとして残した。

**他に同型の破綻が無いかも機械的に確認した**（2 通り）:
1. 隣の駅までの距離が中央値の 6 倍以上に飛ぶ箇所 → 3 本（只見線 只見〜大白川 17.7km 等、
   いずれも実際に駅間が長いだけ）
2. 連続 3 駅で「行って戻る」形（両端の距離が 2 辺の合計の 3 割未満） → 1 本
   （磐越西線 会津若松、実際のスイッチバック）

つまり自動検出では他に該当なし。ただし**鶴見線自身がこの検出には引っかからなかった**
（都市部で駅間が短く、寄り道の距離が小さいため）ので、見落としはありうる。

**注意（既存データへの影響）**: 海芝浦・新芝浦・大川 を含む区間を `auto_鶴見線_東日本旅客鉄道`
として記録済みの旅程があると、その区間は本線に含まれなくなる。04b の解決は
`resolveByServiceLine → resolveServiceTrip → resolveSegments` の 3 段 fallback があるので
描画自体は N02 経由で残る見込みだが、系統単位の集計はずれる。該当する記録があれば記録し直しが要る。

**MCP 側もインデックス再生成が必要**（`node mcp/scripts/build-index.mjs` → `wrangler deploy`）。
再生成後は `浅野→海芝浦` が「鶴見線(海芝浦支線) 3 駅」に解決し、`鶴見→海芝浦` は
「浅野で乗り換え」の候補が出ることを実測確認。

### 検索タイトルに MCP 対応を出す

`<title>` を `乗レコ - 乗りつぶしマップ｜AIチャット(MCP)対応` に、meta description と
og:description にも「AI チャットから話しかけて記録できる」を追記。Google のスニペット反映は
再クロール待ちで数日〜数週間かかる（v453 と同じ。Search Console で促進可）。

---

## 305. v458 — 🤖 マイページに「AI チャットから記録」の案内を出す（MCP の告知導線）

**カテゴリ**: A（ユスケ「乗レコ本体のマイページなどに案内を出すでお願いします」2026-08-29）

v457 の MCP サーバを `ALLOWED_EMAILS` を外して全開放した（ユスケ実施）。ただし**サーバの URL を
知らない人には辿り着けない**ので、本体から見えるようにした。「堀を作らない」(v453〜v456) の延長。

- マイページヘッダの 📦 の隣に **🤖** を追加（ログイン中・ゲストの両方）。押すと案内モーダル
- モーダルの中身: 何ができるか / 話しかけ方の例 /「カスタムコネクタを追加」の 3 手順 /
  接続先 URL とコピーボタン / できないこと（写真・削除）/ Google ログインが要ること /
  記録方法は「AIチャット」として保存されること / 1 日の上限
- 新規 `js/24-mcp-guide.js`（通信は無く、モーダルの開閉と URL コピーだけ）。
  `navigator.clipboard` が使えない環境では URL を選択状態にして手動コピーへ誘導する

### 実機で見て気付いた: モーダルが開かなかった

`classList.add('show')` と書いていたが、本体のモーダルは **`.open`** で表示される
(`noritetsu-map.html` の `.memo-modal.open{display:flex}`)。つまり **🤖 を押しても何も起きない**
状態だった。構文チェックは通り、`window.openMcpModal` も生えていて、クラスは実際に付くので、
**コードを読むだけでは気付けない**タイプの不具合。

Chromium (Playwright) で実際に開いてスクリーンショットを撮って初めて分かった。この環境では
外部 CDN (Leaflet 等) が取れずアプリの初期化は完走しないが、**モーダルは静的 markup なので
`openMcpModal()` を直接呼べば描画を確認できる**。全部が動かない環境でも、確認したい部分だけ
切り出せば目視検証はできる。

v456 の教訓「モック検証も『一度成功』も実利用の順序を再現しなければ検証にならない」と同根で、
今回は「既存の実装に合わせたつもりで、合わせ先を確認していなかった」形。**既存パターンを踏襲する
ときは、踏襲元を必ず開いて確認する。**
...
### 削除と完乗率の 2 ツールを追加（ユスケ「1 と 2 をすすめて」・2026-08-29）

使ってみて「あれ、できないの？」となる 2 つを埋めた。**PWA 側の変更は無いので CACHE_VERSION は据え置き**。

**`delete_trip`** — 間違えて記録したときに AI から消せる。安全側に倒すため:
- 入力は `trip_id` のみ（曖昧な名前では消せない）。id は `list_recent_trips` で調べさせる
- 先に 1 件 SELECT して「何を消すか」を返せるようにし、見つからなければ何もしない
- 写真は Supabase の行を消しても **R2 に残る**ので、本体の削除と同じく `api.norireco.app/delete/photo`
  に本人の access token を付けて掃除する（MCP 側は R2 の鍵を持たないのでこの経路しかない）。
  ベストエフォートで、失敗しても旅程の削除自体は成立させる
- 書き込み枠 (1 日 60 件) を消費する

**`get_completion`** — 「いま何%？」「あと何駅で完乗？」「惜しい路線は？」に答える。
`line` を渡せば 1 系統の進捗だけ返す（「山手線あと何駅？」）。`since` / `until` で期間も絞れる。

**本体と同じ数字を出すことを最優先にした**。マイページの「🚃 完駅率」カードと食い違うと、
どちらを信じればいいのか分からなくなる。そのため `js/13a-stats.js` の `computeCompletionStats()`
をそのまま移植し、**わざと本体と同じ数え方**にしてある:
- 分母は SERVICE_LINES の駅 id の集合（merged_stations 全体ではない）= 9,030 駅
- 区間の展開は**駅名で findIndex**（04b は v422 で id 優先に移ったが、完駅率カードを描いて
  いるのは 13a-stats の方なので、そちらに合わせる）

走行距離を出すためにインデックスへ駅の座標を追加（686 KB / bundle 471 KB gzip）。

検証は fixture で 12 ケース: ユニーク駅の重複排除（立川が中央線快速と青梅線の両方に出ても 1 駅）/
完乗判定 / 残り駅の中身 / 未乗車・完乗系統の扱い / 記録ゼロ / 走行距離の妥当性。
MCP 経由でもツールが 7 つ並ぶこと、未接続時に落ちずに再接続案内が返ることを実測。
