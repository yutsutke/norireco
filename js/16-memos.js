...
function fillModal({ title, sub, memo }) {
  ...
  // 暫定 (v250): photos[0].url を写真URL input にセット。v251+ で R2 アップロード UI に置換
  const firstPhoto = Array.isArray(memo.photos) && memo.photos[0] ? memo.photos[0] : null;
  document.getElementById('m-photo').value = (firstPhoto && firstPhoto.url) || '';
...
function readModal() {
  ...
  // v250 暫定: 写真URL 直接入力を photos[0].url に格納 (R2 連携前)
  const photos = photoUrl ? [{ url: photoUrl }] : [];
  return { memo_type, mood, tags, comment, photos };
}
...
async function saveMemoFromModal() {
  ...
  try {
    ...
    let result;
    if (isEdit) {
      result = await updateMemoOnServer(M.editingId, fields);
      const idx = M.cache.findIndex(m => m.id === M.editingId);
